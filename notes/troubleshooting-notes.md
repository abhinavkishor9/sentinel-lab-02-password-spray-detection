# Troubleshooting Notes — Sentinel Lab 02

## Purpose

This document records the technical issues encountered while building the password-spray detection lab and the changes made to ensure that the lab remains technically accurate and independently executable in Microsoft Sentinel.
...
When using synthetic KQL data in Sentinel:

```text
Do not assume synthetic data is persistent telemetry.
Do not assume a `let` variable survives between queries.
Do not create a production Analytics Rule from temporary
`datatable()` data.
```
...
```text
Use self-contained queries.
Document synthetic data clearly.
Validate each query independently.
Separate detection logic from production alerting.
```
